/**
 * error_fname, error_lname, error_email, error_mobile, error_address, error_date
    ...
    if (pattern.test(email) && email !== "") {
      ...
      if (emailDuplicate == false) {
        ...
        $("#email_error_message").html(
          "Email already exist! Try something else"
        );
    ...
    if (pattern.test(mob)) {
      ...
      if (mobileDuplicate == false) {
        ...
        $("#mobile_error_message").html(
          "Mob. No. already exist! Try something else"
        );
      ...
      $("#mobile_error_message").html(
        "Mobile must contain 10 digit numeric value"
      );
      $("#mobile_error_message").show();
      error_mobile = true;
      $("#mobile").css("color", "tomato");
 ...
 * Seven arguments are passed which are fname, lname, email, mobile, address & date while invocation.
 ...
 */
function autoFill(rowNo, fName, lName, Email, Mobile, Address, date) {
 ...
 */
function updateTable(r, fName, lName, Email, Mobile, Address, date) {
  event.preventDefault();
 ...
 */
function updateRow(indexThis, fName, lName, Email, Mobile, Address, date) {
  update = true;

  var rowNo = indexThis.parentNode.parentNode.rowIndex;

  rowUpdateNo = rowNo;
  autoFill(rowNo, fName, lName, Email, Mobile, Address, date);

  document.getElementById("Heading").innerHTML = "Update Details";
  $("#Heading").html("Update Details");

  error_fname = error_email = error_mobile = false;
  error_lname = error_address = error_date = false;
}
 ...
 */
function addToTable(fName, lName, Email, Mobile, Address, date) {
  ...
  btnUpdate.onclick = function () {
    event.preventDefault();
    updateRow(this, fName, lName, Email, Mobile, Address, date);
  };
...
function resetDet() {
  ...
  error_date = error_fname = error_email = error_mobile = true;
  error_address = error_lname = false;
...
function myFunction() {
  ...
  if (statusFinal == true) {
    if (update == true) {
      update = false;
      updateTable(rowUpdateNo, fName, lName, Email, Mobile, Address, date);
    } else {
      addToTable(fName, lName, Email, Mobile, Address, date);
      resetDet();
    }
  } else {
